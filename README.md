...
## サーバー
- **Redisを利用した高速化**: ユーザー同士（男性×女性）のリアルタイムマッチメイキングを高速化するため、事前に相性をバッチ計算しRedisに保存する方式に変更。100人×100人の組み合わせにおいて、処理時間を450秒から220秒に短縮し、サービスのUX要件を満たした。
...
- **AWS Lambdaを利用した並列化**: 画像処理とLLM呼び出しを含む30秒程度の処理を、分間150リクエストを処理する要件があった。Job実行基盤の水平スケーリング（スレッド数・サーバー台数の増加）で理論的には処理可能な状態を構築したが、MiniMagickによる画像処理が並列化されず、150リクエストの総処理時間が5分を超えてしまう問題が発生していた。ボトルネックとなっていた処理をLambdaへ切り出し、並列実行する構成とすることで、要件を満たす処理時間まで短縮した。
- **ポリモーフィックなモデル設計**: サーバーにおけるUser/Guestの二系統を、共通のAccountableインターフェースとしてポリモーフィックに設計。運用中も負債になりにくく、コード重複を大幅に削減した。
- **Apple/Googleのサブスクリプション実装等**: サーバーの設計（サーバー通知の処理）とクライアント実装を、公式ドキュメントおよび技術検証を行い横断して整備した（実装は主にAndroidを担当し他はディレクション）。リリース後にも実データを検証し、データの信頼性を確認した（[検証ログiOS版](https://zenn.dev/link/comments/4ea2cd272e3784)、[Android版](https://zenn.dev/link/comments/1fa236836939ed)）。同様に、Sign in with Appleも実装とディレクションを行った。
